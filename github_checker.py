...
class Checker:
    ...
    This class is responsible for performing accessibility checks on GitHub website and API,
    and collecting statistics such as response time and success rate
    """
    ...
    def test(self, timeout: float = 8.0) -> dict:
        ...
        for name, url in self.TARGETS:
            ...
            if target_results:
                avg_response = sum(r.get("ms", 0) for r in target_results if "ms" in r) / len(target_results)
                successful_responses = sum(1 for r in target_results if r.get("ok", False))
                target_stats[name] = {
                    "avg_response": avg_response,
                    "success_rate": successful_responses / len(target_results) * 100
                }
    ...
    def check(self, timeout: float = 8.0) -> dict:
        ...
        Returns:
            dict: Dictionary containing detection status, total time, results and message
        """
        ...
        for name, url in self.TARGETS:
            ...
            remain = max(1.0, timeout - elapsed)   # Calculate remaining time, at least 1 second
            
            r = self._test(url, remain)            # Execute single URL test
            results.append((name, r))              # Add result to list
            
            # If homepage detection fails, stop subsequent detection (fast fail)
            if name == "homepage" and not r["ok"]:
                break
        
        total_ms = (time.time() - start) * 1000   # Calculate total time in milliseconds
    ...
    def _test(self, url: str, timeout: float) -> dict:
        ...
        Returns:
            dict: Dictionary containing test results, including success status, response time, status code or error message
        """
        try:
            ...
            # Return success result: status code 200 means success, calculate response time, record status code
            return {
                "ok": resp.status_code == 200,                    # Whether successful
                "ms": round((time.time() - t0) * 1000),           # Response time in milliseconds
                "status_code": resp.status_code                   # HTTP status code
            }
        except requests.exceptions.Timeout:
            # Request timeout exception
            return {"ok": False, "error": "Request timed out", "error_type": "timeout"}
        except requests.exceptions.ConnectionError:
            # Connection error exception
            return {"ok": False, "error": "Connection error - check network", "error_type": "connection"}
        except requests.exceptions.RequestException as e:
            # Other request exceptions
            return {"ok": False, "error": f"Request error: {str(e)}", "error_type": "request"}
        except Exception as e:
            # Other unexpected exceptions
            return {"ok": False, "error": f"Unexpected error: {str(e)}", "error_type": "unknown"}
    ...
    def _judge(self, results: list) -> str:
        ...
        Returns:
            str: Network status ("good", "warn", or "bad")
                 - "good": All targets succeed and average response time less than 3 seconds
                 - "warn": Partial success or average response time greater than or equal to 3 seconds
        ...
        # Return good if average response time less than 3 seconds, otherwise warn
    ...
    def _msg(self, status: str, results: list) -> str:
        ...
        """
        if status == "good":
            # Calculate average response time for good status message
            successful_results = [r for _, r in results if "ms" in r]
            if successful_results:
                avg_time = sum(r["ms"] for _, r in results if "ms" in r) / len(successful_results)
                return f"[OK] GitHub is accessible (avg {avg_time:.0f}ms)"
            else:
                return "[OK] GitHub is accessible"
        elif status == "warn":
            # Determine which targets have issues
            failed_targets = [name for name, r in results if not r.get("ok")]
            if failed_targets:
                return f"[WARN] GitHub is unstable ({', '.join(failed_targets)} affected)"
            else:
                # Calculate average response time for warning message (when response time is too long)
                successful_results = [r for _, r in results if "ms" in r]
                if successful_results:
                    avg_time = sum(r["ms"] for _, r in results if "ms" in r) / len(successful_results)
                    return f"[WARN] GitHub is accessible but slow (avg {avg_time:.0f}ms)"
                else:
                    return "[WARN] GitHub is accessible but slow"
        elif status == "bad":
            # Check for complete failure or partial failure
            failed_targets = [name for name, r in results if not r.get("ok")]
            return f"[FAIL] Cannot connect to GitHub ({', '.join(failed_targets)})"
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="GitHub Network Status Checker")
    ...
    print("=" * 40)
    print("Checking GitHub accessibility...", end=" ")  # Print check start prompt
    ...
    # Initialize spinner_thread as None to avoid reference errors in exception handling
    spinner_thread = None
    
    # For normal checks, display animation until check completes
    # For full test, test method already has progress indicator
    if not args.full_test:
        # Display animation in separate thread for normal check
        def show_spinner():
            for cursor in itertools.cycle('|/-\\'):  # Cycle through spinning cursor characters
                if hasattr(show_spinner, 'done'):     # Check if there's completion flag
                    break
                sys.stdout.write('\b' + cursor)      # Backspace and write new cursor character
                sys.stdout.flush()                   # Flush output buffer
                time.sleep(0.1)                      # 0.1 second delay
        
        spinner_thread = threading.Thread(target=show_spinner)  # Create animation thread
        spinner_thread.daemon = True  # Set as daemon thread
        spinner_thread.start()        # Start animation thread
    ...
    try:
        ...
        # If animation thread is running, stop it
        if not args.full_test and spinner_thread is not None:
            show_spinner.done = True                    # Set completion flag
            spinner_thread.join(timeout=0.2)            # Wait for thread to complete (with timeout)
            print("\r" + " " * 50 + "\r", end="")      # Clear cursor characters
        ...
        if is_full_test:
            ...
            for name, stats in r['target_stats'].items():
                print(f"  {name:10}: Avg {stats['avg_response']:.0f}ms, Success rate: {stats['success_rate']:.1f}%")
        else:
        ...
        if r["status"] == "good":
            ...
            failed_targets = [name for name, result in r.get("results", []) if not result.get("ok")]
            if failed_targets:
                print(f"\nSuggestion: Network is unstable for {', '.join(failed_targets)}. Try again later, or use a proxy/VPN.")
            else:
                print("\nSuggestion: Network is slow but accessible. Consider trying again later or using a proxy/VPN for better performance.")
        else:
            print("\nSuggestion: Network connection failed. Check your internet connection, firewall settings, or use a VPN if needed.")
        ...
        if not args.full_test and spinner_thread is not None:
            ...
            spinner_thread.join(timeout=0.2)  # Wait for thread to complete (with timeout)
        print("\n\nInterrupted by user.")  # Print user interrupt prompt
        return 1
    except Exception as e:
        # If animation thread is running, stop it
        if not args.full_test and spinner_thread is not None:
